<template>
    <el-table-column v-bind="column" v-on="$listeners">
        <template slot="header" slot-scope="scope">
            <s-render v-if="column.renderHeader" :scope="scope" :render="column.renderHeader" />
            <span v-else>{{ scope.column.label }}</span>
        </template>

        <template slot-scope="scope">
            <component v-if="column.component" v-bind="getCptBind(scope, column)" v-on="column.listeners" :is="column.component" />

            <!-- 嵌套表格 -->
            <template v-else-if="column.children">
                <s-column v-for="col in column.children" :key="col.prop" :column="col" />
            </template>

            <s-render v-else :scope="scope" :render="column.render" />
        </template>
    </el-table-column>
</template>
